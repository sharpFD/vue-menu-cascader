<script>
  ...
  export default {
    ...
    methods: {
      ...
      getSpecificNode(nodes, deep) {
          let path = this.node.path[deep]
          for(let i = 0; i< nodes.length; i++) {
              let node = nodes[i]
              if (node.value === path) {
                  if (node.hasChildren) {
                      nodes = node.children
                      return this.getSpecificNode(nodes, ++deep)
                  } else {
                      return node
                  }
              }
      ...
      renderPrefix(h) {
        ...
        if (multiple) {
          return this.renderCheckbox(h, isLeaf);
      ...
      renderContent(h) {
          /*on-mouseleave={() => { panel.currentNode({node: null, data: null}) }}*/
        ...
        return (
          <span class="el-cascader-node__label" on-mousemove={() => { panel.currentNode({node: {
                  ...node,
                  children: null,
                  pathNodes: null
              }, data: node.data}) }}>{ vnode || node.label }</span>
        );
      }
    },
