<template>
    <el-popover :disabled="loading" v-model="show" popper-class="m-pop" @show="showPop" @hide="hidePop"
        placement="bottom-start" transition="el-zoom-in-top" trigger="click">
        ...
        <div :class="['el-cascader-panel']" @keydown="handleKeyDown">
            ...
            <div class="desc">
                <slot name="desc" :scope="nodeData"></slot>
            </div>
        </div>
    </el-popover>
</template>
...
export default {
    ...
    methods: {
        showPop() {
            ...
            let _ = this;
            this.$nextTick(() => {
                _.$refs.menu[0].$refs['m-input'].focus();
                _.$refs.menu[0].$refs['m-input'].focused = true;
            });
        },
        hidePop() {
            this.$emit('hide');
